...
unsigned int sleep(unsigned int seconds)
{
	...
	iom->addTimer(seconds*1000, [fiber, iom](){iom->addscheduletask(fiber, -1);});
...
int usleep(useconds_t usec)
{
	...
	myCoroutine::IOManager* iom = myCoroutine::IOManager::getThis();
	// add a timer to reschedule this fiber
	iom->addTimer(usec/1000, [fiber, iom](){iom->addscheduletask(fiber);});
	// wait for the next resume
	fiber->yield();
	return 0;
}
...
int nanosleep(const struct timespec* req, struct timespec* rem)
{
	...
	myCoroutine::IOManager* iom = myCoroutine::IOManager::getThis();
	// add a timer to reschedule this fiber
	iom->addTimer(timeout_ms, [fiber, iom](){iom->addscheduletask(fiber, -1);});
	// wait for the next resume
	fiber->yield();	
	return 0;
}
